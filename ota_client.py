...
CLIENT_VERSION = "4.0-pairing-clean"
...
.box{{text-align:center;background:#111b31;border:1px solid rgba(255,255,255,.14);border-radius:34px;padding:60px 90px}}
.logo{{font-size:48px;font-weight:950;margin-bottom:18px}}.label{{font-size:30px;color:#b8c7e0;margin:10px 0}}.code{{font-size:118px;font-weight:1000;letter-spacing:14px;color:#8ab4ff;margin:18px 0}}.small{{font-size:18px;color:#7f91ad;margin-top:24px}}
</style></head><body><div class="box"><div class="logo">VäV</div><div class="label">Ny skärm väntar på koppling</div><div class="code">{code}</div><div class="label">Superadmin → Skärmar → koppla koden</div><div class="small">{device_id}<br>{server}</div></div></body></html>"""
    ...
    while True:
        try:
            ...
            if not cfg.get("DEVICE_TOKEN"):
                ...
                requests.post(f"{server}/api/device/register", json={"device_id":device_id,"pairing_code":code,"hostname":socket.gethostname(),"name":socket.gethostname(),"version":CLIENT_VERSION}, timeout=10)
                ...
                data = requests.get(f"{server}/ota/check", headers={"Authorization": f"Bearer {token}"}, timeout=15).json()
